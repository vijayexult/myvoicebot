...
# Configure logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)Ys')

# Initialize colorama
init(autoreset=True)

def receive_audio_data_from_websocket(ws):
    while True:
        try:
            audio_data = ws.recv()
            return audio_data
...
def main():
    ...
    while True:
        try:
            audio_data = receive_audio_data_from_websocket(ws)

            # Get the API key for transcription
            transcription_api_key = get_transcription_api_key()
            
            # Transcribe the audio data
            user_input = transcribe_audio(Config.TRANSCRIPTION_MODEL, transcription_api_key, audio_data, Config.LOCAL_MODEL_PATH)
